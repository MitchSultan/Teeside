      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-5">
            {[
              { icon: Phone, title: 'Phone', info: '+254 722 841 455', sub: 'Mon-Sat, 8am-6pm EAT', href: 'tel:+254700000000' },
              { icon: Mail, title: 'Email', info: 'teesidemanagementltd@gmail.com', sub: 'We reply within 24 hours', href: 'mailto:info@teeside.co.ke' },
              { icon: MapPin, title: 'Office', info: 'Northen Bypass, Nairobi', sub: 'Ergon Plaza, 1st Floor', href: '#' },
              { icon: Clock, title: 'Hours', info: 'Mon-Fri: 8am-6pm', sub: 'Sat: 9am-2pm | Sun: Closed', href: '#' },
              { icon: MessageSquare, title: 'WhatsApp', info: '+254 722 841 455', sub: 'Quick responses guaranteed', href: 'https://wa.me/254700000000' },
              { icon: Building2, title: 'Diaspora Desk', info: '+254 722 841 455', sub: 'Available across all timezones', href: 'tel:+254700000001' },
            ].map((item, i) => (
              <motion.a
                key={item.title}
                href={item.href}
              ...
              <form className="space-y-5" onSubmit={e => e.preventDefault()}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" placeholder="John Doe" className="w-full px-4 py-3 rounded-xl bg-(--color-bg-tertiary) text-sm outline-none focus:ring-2 focus:ring-(--color-gold)/30 transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-(--color-text-secondary) mb-1.5">Phone Number *</label>
                    <input type="tel" placeholder="+254 7XX XXX XXX" className="w-full px-4 py-3 rounded-xl bg-(--color-bg-tertiary) text-sm outline-none focus:ring-2 focus:ring-(--color-gold)/30 transition-all" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-(--color-text-secondary) mb-1.5">Email Address *</label>
                  <input type="email" placeholder="john@example.com" className="w-full px-4 py-3 rounded-xl bg-(--color-bg-tertiary) text-sm outline-none focus:ring-2 focus:ring-(--color-gold)/30 transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-(--color-text-secondary) mb-1.5">I am a...</label>
                  <select className="w-full px-4 py-3 rounded-xl bg-(--color-bg-tertiary) text-sm outline-none text-(--color-text-secondary)">
