...
export default function AboutPage() {
  return (
    <div className="pt-20 lg:pt-24 pb-16">
      {/* Hero */}
      <section className="bg-[var(--color-navy)] py-16 lg:py-24 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
            <span className="text-sm font-semibold text-[var(--color-gold)] uppercase tracking-wider mb-3 block">About Teeside</span>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white leading-tight mb-5" style={{ fontFamily: 'var(--font-inter)' }}>
              Nairobi&apos;s Most Trusted <span className="text-[var(--color-gold)]">Property Partner</span>
            </h1>
            <p className="text-lg text-white/60 leading-relaxed max-w-xl">
              Since 2017, Teeside Properties has been bridging the gap between property owners and tenants across the Nairobi Metropolitan Area — with integrity, innovation, and results.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              ...
              <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="neu-raised p-5 text-center">
                <p className="text-2xl font-bold text-[var(--color-navy)]" style={{ fontFamily: 'var(--font-inter)' }}>{s.value}</p>
      ...
      <section className="py-16 lg:py-24 bg-[var(--color-bg-secondary)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <span className="text-sm font-semibold text-[var(--color-gold)] uppercase tracking-wider mb-2 block">Our Mission</span>
      ...
      <section className="py-16 lg:py-24 bg-[var(--color-bg-primary)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {team.map((t, i) => (
              <motion.div key={t.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="neu-raised p-6 text-center">
                ...
                <h3 className="text-base font-semibold text-[var(--color-text-primary)]" style={{ fontFamily: 'var(--font-inter)' }}>{t.name}</h3>
                <p className="text-xs text-[var(--color-gold)] font-medium mb-2">{t.role}</p>
      ...
      <section className="py-16 bg-[var(--color-navy)]">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4" style={{ fontFamily: 'var(--font-inter)' }}>
            Ready to Get <span className="text-[var(--color-gold)]">Started?</span>
          </h2>
